...
/* Kis összecsukható admin toolbar az üzenőfalon */
function WallAdminBar({ adminTopic, setAdminTopic, onSubmit, nextScheduled }) {
  ...
  return (
    <div className="mt-2">
      ...
      >
        <i className="bi bi-shield-lock-fill" style={{fontSize:"0.65rem"}}></i>
        ADMIN
        <i className={`bi bi-chevron-${open ? "up" : "down"}`} style={{fontSize:"0.55rem"}}></i>
      </button>
      ...
      {open && (
        <div className="mt-2 p-2" style={{
          ...
          <form className="d-flex gap-2 align-items-center flex-wrap" onSubmit={onSubmit}>
            <input
              className="form-control form-control-sm"
              style={{ minWidth: 220, fontSize: "0.82rem" }}
              placeholder="Következő heti téma..."
              value={adminTopic}
              onChange={e => setAdminTopic(e.target.value)}
            />
            <button className="btn btn-neon btn-sm" style={{fontSize:"0.78rem"}}>
              <i className="bi bi-calendar-plus me-1"></i>Ütemezés
            </button>
          </form>
          {nextScheduled && (
            <div className="small-muted mt-1" style={{fontSize:"0.7rem"}}>
              <i className="bi bi-clock me-1"></i>
              Életbe lép: <span className="neon">{nextScheduled}</span>
            </div>
          )}
          <div className="small-muted mt-1" style={{fontSize:"0.68rem", opacity:.65}}>
            <i className="bi bi-info-circle me-1"></i>A téma csak a következő hétfőtől aktív.
          </div>
        </div>
      )}
    </div>
...
window.NGH.pages.Wall = function Wall({ auth }) {
  ...
  const [nextScheduled, setNextScheduled] = React.useState(null);
  ...
  async function load() {
    ...
    try {
      ...
      setTopic(t);
      // Backend: { id, email, message, created_at }
      // Frontend: egységesítsük a megjelenítéshez.
  ...
  async function updateTopic(e) {
    e.preventDefault();
    try {
      const res = await api("/api/weekly-topics/current", { method: "POST", token: auth.token, body: { title: adminTopic } });
      setNextScheduled(res.scheduledFor || null);
      setAdminTopic("");
      window.NGH.toast?.(`✅ Ütemezve a következő hétre (${res.scheduledFor})`, { position: "br" });
    } catch (e) {
  ...
  return (
    <div>
      <SectionTitle icon="bi-chat-dots" title="Üzenőfal" subtitle="Hetente frissülő téma • üzenetlista" />
                    ...
                    <div className="d-flex justify-content-between align-items-start gap-2">
                      <div style={{ minWidth: 0 }}>
                        <div className="small-muted">
                          <span className="pill me-2">{topic?.title || "Heti téma"}</span>
                          <b className="neon">{m.name || m.email || "vendég"}</b> • {formatDate(m.createdAt)}
                        </div>
